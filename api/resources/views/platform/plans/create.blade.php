...
<div class="max-w-3xl mx-auto">
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold">Créer un nouveau palier</h1>
            <p class="text-sm text-slate-500">Configurez les limites et les services inclus.</p>
        </div>
    </div>

    <form action="{{ route('platform.plans.store') }}" method="POST" class="space-y-8">
        @csrf
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl">
            <div class="grid gap-6 md:grid-cols-2">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" required placeholder="ex: Starter, Business..." value="{{ old('name') }}" class="mt-1 block w-full rounded-lg border-slate-800 bg-slate-950 text-white focus:border-indigo-500 @error('name') border-red-500 @enderror">
                    @error('name') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-300">Prix Mensuel (€)</label>
                    <input type="number" step="0.01" name="price_monthly" required value="{{ old('price_monthly', 29) }}" class="mt-1 block w-full rounded-lg border-slate-800 bg-slate-950 text-white focus:border-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-300">Prix Annuel (€)</label>
                    <input type="number" step="0.01" name="price_yearly" required value="{{ old('price_yearly', 290) }}" class="mt-1 block w-full rounded-lg border-slate-800 bg-slate-950 text-white focus:border-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-300">Max Employés (vide = illimité)</label>
                    <input type="number" name="max_employees" value="{{ old('max_employees') }}" placeholder="∞" class="mt-1 block w-full rounded-lg border-slate-800 bg-slate-950 text-white focus:border-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-300">Période d'essai (jours)</label>
                    <input type="number" name="trial_days" required value="{{ old('trial_days', 14) }}" class="mt-1 block w-full rounded-lg border-slate-800 bg-slate-950 text-white focus:border-indigo-500">
                </div>

                <div class="md:col-span-2 py-2">
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" checked value="1" class="h-5 w-5 rounded border-slate-800 bg-slate-950 text-indigo-500 focus:ring-indigo-500">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                @foreach($features as $key => $label)
                    <div class="flex items-center gap-3 p-3 rounded-xl border border-slate-800 bg-slate-950 hover:bg-slate-800/40 transition-colors">
                        <input type="checkbox" name="features[{{ $key }}]" id="feat_{{ $key }}" value="1" {{ old("features.$key") ? 'checked' : '' }} class="h-5 w-5 rounded border-slate-700 bg-slate-900 text-indigo-500 focus:ring-indigo-500">
        ...
        <div class="flex justify-end pt-4">
            <button type="submit" class="rounded-xl bg-indigo-500 px-8 py-3 font-bold text-white shadow-lg shadow-indigo-500/30 hover:bg-indigo-400 transition-all flex items-center gap-2">
                Enregistrer le plan
            </button>
        </div>
    </form>
